...
//! `Skill` grants no capability of its own (it loads instructions, it does
//! not act) and the user's skills reached the session's prompt either way,
//! since neither `--bare` nor `--safe-mode` is usable here; what a skill
//! then asks for is still gated by this same list.
...
mod tests {
    ...
    fn the_backstop_stops_a_session_offered_tools_bancada_did_not_ask_for() {
        let line =
            r#"{"type":"system","subtype":"init","tools":["Read","Write","Bash","Monitor"]}"#;
        ...
        assert!(detail.contains("Monitor"), "{detail}");
    }
