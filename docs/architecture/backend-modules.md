...
| `idfenv.rs` | 700 | Finding and activating an ESP-IDF install, from the records it left behind rather than `IDF_PATH`: the installer registry first, then `install.sh`'s `idf-env.json`. Its rustdoc records the three approaches that do **not** work for installer layouts, each probe-verified — `idf_tools.py export`, a hand-built environment, and sourcing the activation script (which mutates the registry it reads) — and why `idf_tools.py export` *is* the right call for a manual checkout. |
